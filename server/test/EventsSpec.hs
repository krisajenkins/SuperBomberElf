...
scheduleSpec =
  ...
  do it "Join and name should result in one named player" $
       property $
       \(t,uuid,name) ->
         eventsCause [(AddPlayer uuid,t),(SetPlayerName uuid name,addTime 5 t)]
                     (addTime 10 t)
                     (Scene {_players = Map.fromList [(uuid,Player name)]},[])
     it "Removing a non-existent player shouldn't matter." $
       property $
       \(t,uuid) ->
         eventsCause [(RemovePlayer uuid,t)]
                     (addTime 10 t)
                     (Scene Map.empty,[])
     it "Adding n players results in n players." $
       property $
     ...
     it "Double adding a player causes an error." $
       property $
       \(t1,t2,uuid) ->
         eventsCause
     ...
     it "Future events are not processed." $
       property $
       \(t,(e1,e2)) ->
